...
public class CAPKTable
{
	...
	public byte[] getDataBuffer()
	{
		...
		System.arraycopy(ByteConversionUtils.hexStringToByteArray(rid), 0, data, offset+3, rid.length()/2);
		offset += (3+rid.length()/2);
		
		data[offset]   = (byte)0x9F;
		data[offset+1] = 0x22;
		data[offset+2] = 0x01;
		data[offset+3] = ByteConversionUtils.hexStringToByteArray(capki)[0];
		offset += 4;
		...
		System.arraycopy(ByteConversionUtils.hexStringToByteArray(modul), 0, data, offset+4, modul.length()/2);
		offset += (4+modul.length()/2);
		
		data[offset]   = (byte)0xDF;
		data[offset+1] = 0x04;
		data[offset+2] = (byte)(exponent.length()/2);
		System.arraycopy(ByteConversionUtils.hexStringToByteArray(exponent), 0, data, offset+3, exponent.length()/2);
		...
		if(checkSum != null && checkSum.length() > 0)
		{
			...
			System.arraycopy(ByteConversionUtils.hexStringToByteArray(checkSum), 0, data, offset+3, checkSum.length()/2);
